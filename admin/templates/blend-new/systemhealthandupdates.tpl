...
<style>
/* Hide Automatic Update Requirements - manual updates only */
.health-status-col .panel[id*="Updater"],
.health-status-col .panel[id*="updater"] { display: none !important; }
</style>
...
<script>
    jQuery(window).load(function() {
        ...
        var hidden = jQuery('.health-status-col .panel[id*="Updater"], .health-status-col .panel[id*="updater"]');
        hidden.each(function() {
            var col = jQuery(this).closest('.health-status-col');
            var countEl = col.hasClass('health-status-col-danger') ? jQuery('.health-status-block-danger .count') : jQuery('.health-status-block-warning .count');
            var n = parseInt(countEl.text(), 10) - 1;
            countEl.text(Math.max(0, n));
        });
    });
</script>
